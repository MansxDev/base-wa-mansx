...
const { imageToWebp, videoToWebp, writeExif } = require('../system/exif');
const { isUrl, getGroupAdmins, generateMessageTag, getBuffer, getSizeMedia, fetchJson, sleep, getTypeUrlMedia } = require('../system/function');
...
async function MessagesUpsert(conn, message, store) {
	try {
		...
		require('../mansx.js')(conn, m, message, store);
...
conn.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
	...
	conn.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		...
		const trueFileName = attachExtension ? `./src/${filename ? filename : Date.now()}.${type.ext}` : filename;
	...
	conn.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		...
		if (isWebpSticker) {
			const { writeExif } = require('../system/exif');
